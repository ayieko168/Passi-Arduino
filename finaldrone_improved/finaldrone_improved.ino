...
// (tony vars)
#include <avr/sleep.h>  //library that contains the controls for the sleep modes
#define interruptPin 2  //The pin we will use to wake up the duio

int buttonPin = 6;      //The pin we will use to put the duino to sleep
int buttonNew;
int buttonOld = 1;
int debounce = 100;

volatile int arduino_status = 12;
...
void setup()
{
  ...
  //(tony setup)
  pinMode(interruptPin, INPUT_PULLUP);  //Set the D2 pin to an input but with an internal pullup pin i.e LOW when it's pressed
  pinMode(buttonPin, INPUT_PULLUP);     //Set the D6 pin to an input but with an internal pullup pin i.e LOW when it's pressed
  pinMode(arduino_status, OUTPUT);      //This pin has an led that blinks once every cycle when not in sleep and blinks rapidly when going to sleep and finaly shuts off when 
...
{ 
  ...
  buttonNew = digitalRead(buttonPin);   // Check for a sleep button press
  Serial.print("Sleep Button state : ");
  Serial.println(buttonNew);

  //Use the led indicator
  if(buttonNew == 1){  //If the duino is on (not in sleep)
      digitalWrite(arduino_status, LOW);
      digitalWrite(arduino_status, HIGH);
      delay(250);
      digitalWrite(arduino_status, LOW);
  }else{
     for(int i = 0; i<10; i++){
